...
public class ActionGUI extends JFrame {
    ...
    public ActionGUI(Socket soc, String uname, Socket sSoc) {
        ...
        String[] columnNames = {"Symbol", "Price", "Profit"}; // Define column names
        itmmodl = new ItemModel(itmLst, columnNames); // Instantiate ItemModel with column names
        setSubTable(itmmodl); 
        setResizable(false);
    ...
    public synchronized void createITMTablemodel(Socket pSoc) {
        itmLst = sublst.getAlrdySubItemWithDitail(pSoc, uName); 
        String[] columnNames = {"Symbol", "Price", "Profit"};// get already subscribed Symbol list with it's details from server
        itmmodl = new ItemModel(itmLst,columnNames); // set model
        setSubTable(itmmodl);
    }
